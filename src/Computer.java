...
public class Computer {
	...
	public void executeNot() {
		...
		char[] ccValue = mRegisters[destBS.getValue()].getValue2sComp() > 0 ? new char[] {'0','0','1'} 
						:mRegisters[destBS.getValue()].getValue2sComp() == 0 ? new char[] {'0','1','0'}
						:new char[] {'1','0','0'};
		this.mCC.setBits(ccValue);
	}
	/**
	 * 
	 */
	public void executeAdd() {
		...
		BitString source2BS = mIR.substring(12, 3);
		BitString imm5BS = mIR.substring(10, 5);
		int sum = 0;
		//add sr1 to sr2 and store it to destBS
		if (mIR.substring(9, 1).getValue() == 0) {
			sum = mRegisters[source1BS.getValue()].getValue2sComp() + mRegisters[source2BS.getValue()].getValue2sComp();
		}
		//add n value to sr1 and store it to destBS
		else {
			sum = mRegisters[source1BS.getValue()].getValue2sComp() + imm5BS.getValue2sComp();
		}
